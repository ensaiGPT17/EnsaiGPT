    ...
    def choisir_menu(self):
        ...
        if choix == "Changer nom d'utilisateur":
            ...
            if res.code == 200:
                ...
                new_username = inquirer.text(message="Rentrez votre nouveau nom "
                                                     "d'utilisateur:").execute()
                res_chgt_username = user_service.change_username(user.id_user,
                                                                 new_username)
                ...
                from view.userviews.main_menu_view import MainMenuView
                return MainMenuView("Mot de passe erroné! Retour au Menu Principal")

        elif choix == "Changer mon mot de passe":
            # demander mdp
            password = inquirer.secret(message="Rentrer votre mot de passe "
                                               "actuel:").execute()
            res = user_service.authenticate(username, password)
            if res.code == 200:
                # user confirmé, il peut changer le MOT DE PASSE
                new_password = inquirer.secret(message="Rentrez votre nouveau mot de "
                                                       "passe: ").execute()
                new_password_confirm = inquirer.secret(message="Confirmez le mot de "
                                                               "passe : ").execute()
                if new_password != new_password_confirm:
                    from view.userviews.main_menu_view import MainMenuView
                    return MainMenuView(f"Erreur : Les mots de passe ne correspondent "
                                        f"pas.")

                res_chgt_password = user_service.change_password(user.id_user,
                ...
                if res_chgt_password.code==200:
                    ...
                    from view.userviews.main_menu_view import MainMenuView
                    message = f"Échec:{res_chgt_password.content}\n"
                    return MainMenuView(message)
                elif res_chgt_password.code==500:
                    from view.userviews.main_menu_view import MainMenuView
                    message = f"Échec, erreur inconnue:{res_chgt_password.content}\n"
                    return MainMenuView(message)
            else:
                from view.userviews.main_menu_view import MainMenuView
                return MainMenuView("Mot de passe erroné! Retour au Menu Principal")
